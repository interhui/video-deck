...
csv-movie-001,CSV电影,CSV描述,CSV电影,演员A|演员B,CSV导演,2024,CSV发行商,120,标签A|标签B,D:/Movies/csv.mp4,H264,1920,1080,7200`;
    ...
    describe('scanMovieDirectory - 视频扫描模式', () => {
        test('SVC-MOVIE-052: 视频扫描模式-正常情况', async () => {
            // 创建视频目录和视频文件
            const videoDir = path.join(testDataDir, 'videoScan');
            fs.mkdirSync(videoDir, { recursive: true });

            // 创建测试视频文件
            fs.writeFileSync(path.join(videoDir, 'Video1.mp4'), 'video content 1');
            fs.writeFileSync(path.join(videoDir, 'Video2.mkv'), 'video content 2');

            const result = await service.scanMovieDirectory(videoDir, 'video', 'movie', moviesDir);

            expect(result.success).toBe(true);
            expect(result.movies.length).toBe(2);

            // 验证电影ID和电影名都使用视频文件名称命名
            const movieIds = result.movies.map(m => m.movieData.id).sort();
            const titles = result.movies.map(m => m.movieData.title).sort();
            expect(movieIds).toEqual(['Video1', 'Video2']);
            expect(titles).toEqual(['Video1', 'Video2']);
        });

        test('SVC-MOVIE-053: 视频扫描模式-original_filename填入完整路径', async () => {
            const videoDir = path.join(testDataDir, 'videoOrigin');
            fs.mkdirSync(videoDir, { recursive: true });

            fs.writeFileSync(path.join(videoDir, 'Test.Movie.mp4'), 'video');
            ...
            // 验证 original_filename 填入了完整路径
            const movie = result.movies[0];
            expect(movie.movieData.original_filename).toContain('Test.Movie.mp4');
            expect(movie.movieData.original_filename).toContain(videoDir);
        });

        test('SVC-MOVIE-054: 视频扫描模式-电影ID为目录名', async () => {
            const videoDir = path.join(testDataDir, 'videoDirName');
            fs.mkdirSync(videoDir, { recursive: true });

            fs.writeFileSync(path.join(videoDir, 'ID123.mp4'), 'video');

            const result = await service.scanMovieDirectory(videoDir, 'video', 'movie', moviesDir);

            expect(result.success).toBe(true);
            expect(result.movies.length).toBe(1);
            expect(result.movies[0].folderName).toBe('ID123');
        });

        test('SVC-MOVIE-055: 视频扫描模式-空目录', async () => {
            const videoDir = path.join(testDataDir, 'emptyVideoDir');
            fs.mkdirSync(videoDir, { recursive: true });
        ...
        test('SVC-MOVIE-056: 视频扫描模式-无视频文件', async () => {
            const dir = path.join(testDataDir, 'noVideoDir');
            fs.mkdirSync(dir, { recursive: true });
            fs.writeFileSync(path.join(dir, 'readme.txt'), 'readme');

            const result = await service.scanMovieDirectory(dir, 'video', 'movie', moviesDir);

            expect(result.success).toBe(true);
            expect(result.movies.length).toBe(0);
        });

        test('SVC-MOVIE-057: 视频扫描模式-不使用ffmpeg时videoCodec等字段为空', async () => {
            const videoDir = path.join(testDataDir, 'videoNoFf');
            fs.mkdirSync(videoDir, { recursive: true });
            fs.writeFileSync(path.join(videoDir, 'TestVideo.mp4'), 'video');

            const result = await service.scanMovieDirectory(videoDir, 'video', 'movie', moviesDir);

            expect(result.success).toBe(true);
            expect(result.movies.length).toBe(1);
            expect(result.movies[0].movieData.videoCodec).toBe('');
            expect(result.movies[0].movieData.videoWidth).toBe('');
            expect(result.movies[0].movieData.videoHeight).toBe('');
            expect(result.movies[0].movieData.videoDuration).toBe('');
        });
    });
...
<movie>
    ...
    describe('excludeMoviesInBox', () => {
        test('SVC-MOVIE-062: 排除盒子中已收藏的电影', async () => {
            await service.refreshCache(moviesDir);

            const testMovies = [
                { id: 'movie-1', title: 'Movie A' },
                { id: 'movie-2', title: 'Movie B' },
                { id: 'movie-3', title: 'Movie C' }
            ];

            const excludedIds = ['movie-2'];
            const result = service.excludeMoviesInBox(testMovies, excludedIds);

            expect(result.length).toBe(2);
            expect(result.find(m => m.id === 'movie-1')).toBeDefined();
            expect(result.find(m => m.id === 'movie-2')).toBeUndefined();
            expect(result.find(m => m.id === 'movie-3')).toBeDefined();
        });

        test('SVC-MOVIE-063: 空排除列表返回原电影列表', async () => {
            await service.refreshCache(moviesDir);
            ...
            const result = service.excludeMoviesInBox(testMovies, []);

            expect(result.length).toBe(2);
            expect(result).toEqual(testMovies);
        });

        test('SVC-MOVIE-064: 空电影列表返回空列表', async () => {
            await service.refreshCache(moviesDir);

            const result = service.excludeMoviesInBox([], ['movie-1']);

            expect(result.length).toBe(0);
        });

        test('SVC-MOVIE-065: 排除多个已收藏的电影', async () => {
            await service.refreshCache(moviesDir);
            ...
            const testMovies = [
                ...
                { id: 'movie-2', title: 'Movie B' },
                { id: 'movie-3', title: 'Movie C' },
                { id: 'movie-4', title: 'Movie D' }
            ];

            const excludedIds = ['movie-2', 'movie-4'];
            const result = service.excludeMoviesInBox(testMovies, excludedIds);
            ...
            expect(result.find(m => m.id === 'movie-2')).toBeUndefined();
            expect(result.find(m => m.id === 'movie-3')).toBeDefined();
            expect(result.find(m => m.id === 'movie-4')).toBeUndefined();
        });

        test('SVC-MOVIE-066: null排除列表返回原电影列表', async () => {
            await service.refreshCache(moviesDir);
            ...
            const result = service.excludeMoviesInBox(testMovies, null);

            expect(result.length).toBe(1);
            expect(result).toEqual(testMovies);
        });

        test('SVC-MOVIE-067: undefined排除列表返回原电影列表', async () => {
            await service.refreshCache(moviesDir);
            ...
            const result = service.excludeMoviesInBox(testMovies, undefined);
