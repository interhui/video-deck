...
### 2. Box Commands (电影收藏夹命令)

#### 2.1 box list - 列出收藏夹

| 用例ID | 用例名称 | 测试方法 | 前置条件 | 预期结果 |
|--------|----------|----------|----------|----------|
| CLI-BOX-LIST-001 | 列出所有收藏夹 | 直接调用 | 存在收藏夹 | 输出收藏夹列表 |
| CLI-BOX-LIST-002 | JSON格式 | 直接调用 | 存在收藏夹 | JSON格式输出 |
| CLI-BOX-LIST-003 | 空收藏夹列表（边界条件） | 直接调用 | 无收藏夹数据 | 输出空列表，显示"共 0 个收藏夹" |
| CLI-BOX-LIST-004 | 列表服务异常处理 | 直接调用 | 服务异常 | 错误提示"获取收藏夹列表失败" |
| CLI-BOX-LIST-005 | 别名ls | 直接调用 | 存在收藏夹 | 与list命令等效 |

#### 2.2 box show - 显示收藏夹详情

| 用例ID | 用例名称 | 测试方法 | 前置条件 | 预期结果 |
|--------|----------|----------|----------|----------|
| CLI-BOX-SHOW-001 | 显示存在的收藏夹 | 直接调用 | 收藏夹存在 | 显示收藏夹详情和电影列表 |
| CLI-BOX-SHOW-002 | 显示不存在的收藏夹 | 直接调用 | 收藏夹不存在 | 错误提示"收藏夹不存在" |
| CLI-BOX-SHOW-003 | 收藏夹无电影时显示详情 | 直接调用 | 收藏夹存在但无电影 | 显示收藏夹详情，电影数为0 |
| CLI-BOX-SHOW-004 | 收藏夹详情JSON格式输出 | 直接调用 | 收藏夹存在 | JSON格式输出 |
| CLI-BOX-SHOW-005 | 收藏夹详情服务异常处理 | 直接调用 | 服务异常 | 错误提示获取详情失败 |
| CLI-BOX-SHOW-006 | 别名info | 直接调用 | 收藏夹存在 | 与show命令等效 |

#### 2.3 box create - 创建收藏夹

| 用例ID | 用例名称 | 测试方法 | 前置条件 | 预期结果 |
|--------|----------|----------|----------|----------|
| CLI-BOX-CREATE-001 | 基本创建 | 直接调用 | 无 | 收藏夹创建成功 |
| CLI-BOX-CREATE-002 | 带描述创建 | 直接调用 | 无 | 收藏夹创建成功，描述正确保存 |
| CLI-BOX-CREATE-003 | 创建已存在收藏夹 | 直接调用 | 收藏夹已存在 | 错误提示"收藏夹已存在" |
| CLI-BOX-CREATE-004 | 空收藏夹名称（边界条件） | 直接调用 | 无 | 错误提示名称不能为空 |
| CLI-BOX-CREATE-005 | 超长收藏夹名称 | 直接调用 | 无 | 收藏夹创建成功（名称截断或保存） |
| CLI-BOX-CREATE-006 | 特殊字符收藏夹名称 | 直接调用 | 无 | 收藏夹创建成功 |
...
| 用例ID | 用例名称 | 测试方法 | 前置条件 | 预期结果 |
|--------|----------|----------|----------|----------|
| CLI-BOX-EDIT-001 | 修改名称 | 直接调用 | 收藏夹存在 | 名称已更新，JSON文件已修改 |
| CLI-BOX-EDIT-002 | 修改description字段 | 直接调用 | 收藏夹存在 | 描述已更新，JSON文件已修改 |
| CLI-BOX-EDIT-003 | 修改不存在的收藏夹 | 直接调用 | 收藏夹不存在 | 错误提示"收藏夹不存在" |
| CLI-BOX-EDIT-004 | 重命名为已存在名称 | 直接调用 | 目标名称已存在 | 错误提示"新名称已存在" |
| CLI-BOX-EDIT-005 | 编辑服务异常处理 | 直接调用 | 服务异常 | 错误提示编辑失败 |

#### 2.5 box delete - 删除收藏夹

| 用例ID | 用例名称 | 测试方法 | 前置条件 | 预期结果 |
|--------|----------|----------|----------|----------|
| CLI-BOX-DELETE-001 | 删除存在的收藏夹 | 直接调用 | 收藏夹存在 | 收藏夹已删除 |
| CLI-BOX-DELETE-002 | 删除不存在的收藏夹 | 直接调用 | 收藏夹不存在 | 错误提示"收藏夹不存在" |
| CLI-BOX-DELETE-003 | 使用--force强制删除 | 直接调用 | 收藏夹存在 | 直接删除，无确认提示 |
| CLI-BOX-DELETE-004 | 删除服务异常处理 | 直接调用 | 服务异常 | 错误提示删除失败 |
| CLI-BOX-DELETE-005 | 别名rm | 直接调用 | 收藏夹存在 | 与delete命令等效 |

#### 2.6 box add - 添加电影到收藏夹

| 用例ID | 用例名称 | 测试方法 | 前置条件 | 预期结果 |
|--------|----------|----------|----------|----------|
| CLI-BOX-ADD-001 | 添加电影到收藏夹 | 直接调用 | 收藏夹和电影存在 | 电影已添加到收藏夹，JSON文件已修改 |
| CLI-BOX-ADD-002 | 添加不存在的电影 | 直接调用 | 电影不存在 | 错误提示"电影不存在" |
| CLI-BOX-ADD-003 | 添加到不存在的收藏夹 | 直接调用 | 收藏夹不存在 | 错误提示"收藏夹不存在" |
| CLI-BOX-ADD-004 | 重复添加同一电影到收藏夹 | 直接调用 | 电影已在收藏夹中 | 错误提示"电影已在收藏夹中" |
| CLI-BOX-ADD-005 | 无效电影ID格式 | 直接调用 | 无 | 错误提示电影不存在 |
| CLI-BOX-ADD-006 | 添加服务异常处理 | 直接调用 | 服务异常 | 错误提示添加失败 |

#### 2.7 box remove - 从收藏夹移除电影

| 用例ID | 用例名称 | 测试方法 | 前置条件 | 预期结果 |
|--------|----------|----------|----------|----------|
| CLI-BOX-REMOVE-001 | 从收藏夹移除电影 | 直接调用 | 电影在收藏夹中 | 电影已从收藏夹移除，JSON文件已修改 |
| CLI-BOX-REMOVE-002 | 移除不在收藏夹中的电影 | 直接调用 | 电影不在收藏夹中 | 错误提示"电影不在收藏夹中" |
| CLI-BOX-REMOVE-003 | 从不存在的收藏夹移除电影 | 直接调用 | 收藏夹不存在 | 错误提示"收藏夹不存在" |
| CLI-BOX-REMOVE-004 | 无效电影ID格式 | 直接调用 | 无 | 错误提示电影不在收藏夹中 |
...
#### 8.7 outputBoxList - 收藏夹列表输出

| 用例ID | 用例名称 | 测试方法 | 前置条件 | 预期结果 |
|--------|----------|----------|----------|----------|
| CLI-OUTPUT-BOXLIST-001 | 空收藏夹列表输出 | 直接调用 | 无收藏夹数据 | 输出空表格，显示"共 0 个收藏夹" |
| CLI-OUTPUT-BOXLIST-002 | 有收藏夹列表输出 | 直接调用 | 收藏夹数据存在 | 输出收藏夹表格 |
